...
CAutoManualFindRelation::CAutoManualFindRelation(int disWidth, int disHeight,
		int row, int col) :
		m_disWidth(disWidth), m_disHeight(disHeight), m_row(row), m_col(col) {
...
void CAutoManualFindRelation::putPointsInBlock(
...
void CAutoManualFindRelation::handleKeyPoints(
		std::vector<KeyPoint>& keypoints) {
	FEATUREPOINT_T tmp;
...
void CAutoManualFindRelation::drawPoints(cv::Mat drawMat,
		std::vector<FEATUREPOINT_T>& featurePoints, bool bDraw) {
	for (int i = 0; i < featurePoints.size(); i++) {
		...
		cv::circle(drawMat, featurePoints[i].pixel, 3, cvScalar(0, 0, 0, 0), 2,
				8, 0);
}
...
void CAutoManualFindRelation::insertVertexAndPosition(
	vector<FEATUREPOINT_T> insert) {
fpassemble.clear();
fpassemble = insert;
updateSubdiv();
return;
...
int CAutoManualFindRelation::Point2getPos(const Point2i inPoint,
	Point2i &result) {
int ret = 0;
...
void CAutoManualFindRelation::getTriangleVertex(Point2f fp,
...
void CAutoManualFindRelation::InterpolationPos(Point2i inPoint,
	vector<Point2i>& triVertex, vector<Point2i>& triPos, Point2i& result) {
...
void CAutoManualFindRelation::getPos(Point2i inPoint,
	vector<Point2i>& triVertex, vector<Point2i>& triPos, Point2i& result) {
...
int CAutoManualFindRelation::draw_point_triangle(Mat& img, Point2i fp,
	vector<FEATUREPOINT_T> &back, Point2i &pos, bool bdraw) {
int e0 = 0, vertex = 0;
int num = 0;
int ret = 0;
int flag;
...
void CAutoManualFindRelation::draw_subdiv_point(Mat& img, Point2i fp,
	Scalar color) {
circle(img, fp, 3, color, CV_FILLED, 8, 0);
}

bool CAutoManualFindRelation::readParams(
	std::vector<FEATUREPOINT_T>& getParam) {
char paramName[40];
